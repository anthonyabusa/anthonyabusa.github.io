...
On June 9, Anthropic released Claude Fable 5 to the general public. Fable was the safety-gated version of its Mythos-class model: the same underlying system, wrapped in a different perimeter, with classifiers quietly routing the highest-risk requests into a more conservative lane. It was not the first powerful model the public had touched, and it was not Mythos itself, which stayed restricted. But it crossed the threshold builders actually care about: capable enough, available enough, and familiar enough that people could stop admiring the machine and start building through it. For roughly three days, the distance between thinking of a thing and making the thing got meaningfully shorter.

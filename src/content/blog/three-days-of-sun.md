...
On June 9, Anthropic released Claude Fable 5 to the general public. Fable was the safety-gated version of its Mythos-class model: the same underlying system, but running inside a tighter perimeter. The highest-risk requests were quietly caught by classifiers and routed into a more conservative lane, which meant the public was not getting Mythos itself, and it was not getting the first powerful model humanity had ever touched. But that was never the real line. Fable crossed the line builders actually care about: capable enough to change the work, available enough to use under pressure, ordinary enough that the machine stopped presenting as a demo and started behaving like infrastructure. For roughly three days, the distance between thinking the thing and making the thing got materially smaller.

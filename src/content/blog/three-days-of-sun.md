...
On June 9, Anthropic released Claude Fable 5 to the general public. Fable was the safety-gated version of its Mythos-class model: the same underlying system, but with a different perimeter around it, where classifiers quietly routed the highest-risk requests into a more conservative lane. It was not the first powerful model the public had touched. It was not Mythos itself, which stayed restricted. But it crossed the threshold builders actually care about: capable enough, available enough, and ordinary enough that people could stop staring at the machine from across the room and start building through it. For roughly three days, the distance between imagining the thing and making the thing became meaningfully shorter.

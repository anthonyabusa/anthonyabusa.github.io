...
The first time you get access to a frontier model, the temptation is to hand it the whole shop. It is the most capable thing in the room, or at least the thing most fluent in capability theater, so the serious answer starts to feel obvious: route everything through it, let raw intelligence chew through the mess, and treat the invoice as the price of working near the edge.

I ran my system that way for about a week. Every draft, every file rename, every three-line summary, every procedural splinter went through the strongest and most expensive model I had. A week was enough for the premise to prosecute itself; quietly at first, then with less patience, because eventually the bill stops looking like a receipt and starts looking like telemetry.
...
There is a real risk here, and pretending otherwise would recreate the overconfidence the architecture is supposed to correct. The failure mode is not cheap hands producing sloppy work. The failure mode is a judgment call getting routed *down*, sent to the volume tier because it looked like toil when it was not.
